...
| Arbitrage scan | `ArbitrageScanner` (`src/lib/arbitrage/scanner.ts`) | Deterministic daily eBay-vs-Amazon pairs from the mirror sandbox catalog | **Built**: eBay Browse + Rainforest search matching (`src/lib/arbitrage/real-scanner.ts`), selected when `RAINFOREST_API_KEY` is set and `EBAY_ENV=PRODUCTION`; strict model/variant/size/quantity identity checks are always applied, with optional multimodal AI verification when `OPENAI_API_KEY` is set (`OPENAI_MATCH_MODEL` defaults to `gpt-5.6-luna`). Day-cached in `ScanCache` (~1 Rainforest credit per candidate examined). Demand column is estimated pending Marketplace Insights API access |
