...
export function MobileAppHeader({ user }: { user: { name: string; email: string; isAdmin: boolean } }) {
  ...
  return <><header className="mobile-topbar sticky top-0 z-50 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl md:hidden"><div className="flex h-14 items-center justify-between px-4"><Link href="/dashboard" className="text-[17px] font-bold tracking-tight text-slate-950">Sell<span className="text-indigo-600">finity</span></Link><button type="button" onClick={() => setOpen(true)} aria-label="Open navigation" className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-sm active:scale-95"><span className="text-xs font-bold">{user.name.trim().slice(0, 1).toUpperCase()}</span></button></div></header>
